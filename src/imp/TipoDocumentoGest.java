...
public class TipoDocumentoGest {
   public TipoDocumentoGest(){
   ...
   private Tipodocumento tipodoc=null; //guarda el objeto de la ultima busqueda
   ...
   public Tipodocumento GetObjTipodocumento(){
    ...
    public boolean cargarCamposPorDefecto(){
        try{
            ...
            tipodoc.setDescriptipodoc("CÉDULA");
            sesion.saveOrUpdate(tipodoc);
            tipodoc=new Tipodocumento();
            tipodoc.setIdtipodoc(2);
            tipodoc.setDescriptipodoc("PASAPORTE");
            sesion.saveOrUpdate(tipodoc);
            tx1.commit();
            CERRARSesion();
            return true;
    ...
    public int getsiguientecodigo(){
        ...
        try{
            ...
            Query q= sesion.createQuery("select count(idtipodoc)from Tipodocumento");
            if(Integer.parseInt(q.list().get(0).toString())==0){
                siguientecodigo=1;
            } else {
                Query qq= sesion.createQuery("select max(idtipodoc)from Tipodocumento");
...
public boolean addTipodocumento(Tipodocumento obj){
    ...
    public boolean editTipodocumento(Tipodocumento obj){
      //devuelve true si la transaccion es exitosa
      //false si no lo es
    ...
    public boolean delTipodocumento(Tipodocumento obj){
    ...
    try{
        ...
        Query q=sesion.createQuery("from Tipodocumento where idtipodoc = "+Integer.toString(id));
        if(q.list().isEmpty()){  //si no se encontro
            ...
            existe=true;
            this.tipodoc=(Tipodocumento)q.list().get(0); //el primer elemento de la lista
