...
public class TipohabGest {
   ...
   private Tipohab tipoh=null; //guarda el objeto de la ultima busqueda

   public boolean ExisteTipoHabCargado(){
        sesion = NewHibernateUtil.getSessionFactory().getCurrentSession();
        sesion.beginTransaction();
        Query q=sesion.createQuery("from Tipohab");
   ...
   public Tipohab GetObjTipohab(){
    ...
    public int getsiguientecodigo(){
        ...
        try{
            ...
            Query q= sesion.createQuery("select count(idtipoh)from Tipohab");
            if(Integer.parseInt(q.list().get(0).toString())==0){
                siguientecodigo=1;
            } else {
                Query qq= sesion.createQuery("select max(idtipoh)from Tipohab");
...
public boolean addTipohab(Tipohab obj){
    ...
    public boolean editTipohab(Tipohab obj){
      //devuelve true si la transaccion es exitosa
      //false si no lo es
    ...
    public boolean delTipohab(Tipohab obj){
    ...
    try{
        ...
        Query q=sesion.createQuery("from Tipohab where idtipoh = "+Integer.toString(id));
        if(q.list().isEmpty()){  //si no se encontro
            existe=false;
            this.tipoh=new Tipohab();
        }
        else{
            existe=true;
            this.tipoh=(Tipohab)q.list().get(0); //el primer elemento de la lista
