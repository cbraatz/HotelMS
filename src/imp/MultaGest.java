...
public class MultaGest {
   ...
   private Multa mul=null; //guarda el objeto de la ultima busqueda
   ...
   public Multa GetObjMulta(){
    ...
    public int getsiguientecodigo(){
        ...
        try{
            ...
            Query q= sesion.createQuery("select count(idmulta)from Multa");
            if(Integer.parseInt(q.list().get(0).toString())==0){
                siguientecodigo=1;
            } else {
                Query qq= sesion.createQuery("select max(idmulta)from Multa");
...
public boolean addMulta(Multa obj){
    ...
    public boolean editMulta(Multa obj){
      //devuelve true si la transaccion es exitosa
      //false si no lo es
    ...
    public boolean delMulta(Multa obj){
    ...
    try{
        ...
        Query q=sesion.createQuery("from Multa where idmulta = "+Integer.toString(id));
        if(q.list().isEmpty()){  //si no se encontro
            ...
            existe=true;
            this.mul=(Multa)q.list().get(0); //el primer elemento de la lista
