...
public class CliexsubGest{
    Session sesion=null;
    Cliexsub clxsu=null;
    public void CliexsubGest(){  //constructor
        sesion = NewHibernateUtil.getSessionFactory().getCurrentSession();
    }
    public boolean ExisteFKCliente(int id){
        sesion = NewHibernateUtil.getSessionFactory().getCurrentSession();
        sesion.beginTransaction();
        Query q=sesion.createQuery("from Cliexsub where idcli = "+Integer.toString(id));
    ...
    public boolean ExisteFKSubreserva(int id){
        sesion = NewHibernateUtil.getSessionFactory().getCurrentSession();
        sesion.beginTransaction();
        Query q=sesion.createQuery("from Cliexsub where idsubres = "+Integer.toString(id));
        if(q.list().isEmpty()==false)
            return true;
        else
            return false;
    ...
    public int getsiguientecodigo(){
        ...
        try{
            ...
            Query q= sesion.createQuery("select count(idcliexsub)from Cliexsub");
            if(Integer.parseInt(q.list().get(0).toString())==0)
                siguientecodigo=1;
            else{
                Query qq= sesion.createQuery("select max(idcliexsub)from Cliexsub");
    ...
    public boolean existeClienteEnSubreserva(int idclie, int idsu){
        sesion = NewHibernateUtil.getSessionFactory().openSession();
        boolean encontrado=false;
        Transaction tx2=null;
        try{
           tx2=sesion.beginTransaction();
           Query qq=sesion.createQuery("from Cliexsub where idcli = "+idclie+" and idsubres = "+idsu);
           if(qq.list().size()>0){ //si no se encontro
                encontrado=true;
                clxsu=(Cliexsub) qq.list().get(0);
           }else{
                encontrado=false;
                clxsu=new Cliexsub();
           }
