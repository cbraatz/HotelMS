...
public class UnidadMedidaGest {

   public UnidadMedidaGest(){
   ...
   private Unidadmedida um=null; //guarda el objeto de la ultima busqueda
   ...
   public Unidadmedida GetObjUnidadmedida(){
    ...
    public boolean cargarCamposPorDefecto(){
        try{
            ...
            um=new Unidadmedida();
            um.setIdum(1);
            um.setDescripum("UNIDADES");
            sesion.saveOrUpdate(um);
            um=new Unidadmedida();
            um.setIdum(2);
            um.setDescripum("KILOGRAMOS");
            sesion.saveOrUpdate(um);
            um=new Unidadmedida();
            um.setIdum(3);
            um.setDescripum("GRAMOS");
            sesion.saveOrUpdate(um);
            um=new Unidadmedida();
            um.setIdum(4);
            um.setDescripum("LITROS");
            sesion.saveOrUpdate(um);
            tx1.commit();
            CERRARSesion();
            return true;
    ...
    public int getsiguientecodigo(){
        ...
        try{
            ...
            Query q= sesion.createQuery("select count(idum)from Unidadmedida");
            if(Integer.parseInt(q.list().get(0).toString())==0){
                siguientecodigo=1;
            } else {
                Query qq= sesion.createQuery("select max(idum)from Unidadmedida");
...
public boolean addUnidadmedida(Unidadmedida obj){
    ...
    public boolean editUnidadmedida(Unidadmedida obj){
      //devuelve true si la transaccion es exitosa
      //false si no lo es
    ...
    public boolean delUnidadmedida(Unidadmedida obj){
    ...
    try{
        ...
        Query q=sesion.createQuery("from Unidadmedida where idum = "+Integer.toString(id));
        if(q.list().isEmpty()){  //si no se encontro
            ...
            existe=true;
            this.um=(Unidadmedida)q.list().get(0); //el primer elemento de la lista
