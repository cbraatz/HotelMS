...
public class DescartesGest {

   public DescartesGest(){
   ...
   private Descartes des=null; //guarda el objeto de la ultima busqueda

   public Descartes GetObjDescartes(){
       return des;
   }
    ...
    public boolean ExisteFKMotivoDesc(int id){
        sesion = NewHibernateUtil.getSessionFactory().getCurrentSession();
        sesion.beginTransaction();
        Query q=sesion.createQuery("from Descartes where idmotivo = "+Integer.toString(id));
    ...
    public boolean ExisteFKStockyServ(int id){
        sesion = NewHibernateUtil.getSessionFactory().getCurrentSession();
        sesion.beginTransaction();
        Query q=sesion.createQuery("from Descartes where codigo = "+Integer.toString(id));
        if(q.list().isEmpty()==false)
            return true;
        else
            return false;
    ...
    public int getsiguientecodigo(){
        ...
        try{
            ...
            Query q= sesion.createQuery("select count(iddescarte)from Descartes");
            if(Integer.parseInt(q.list().get(0).toString())==0){
                siguientecodigo=1;
            } else {
                Query qq= sesion.createQuery("select max(iddescarte)from Descartes");
...
public boolean addDescartes(Descartes obj){
    ...
    public boolean editDescartes(Descartes obj){
      //devuelve true si la transaccion es exitosa
      //false si no lo es
    ...
    public boolean delDescartes(Descartes obj){
    ...
    try{
        ...
        Query q=sesion.createQuery("from Descartes where iddescarte = "+Integer.toString(id));
        if(q.list().isEmpty()){  //si no se encontro
            existe=false;
            this.des=new Descartes();
        }
        else{
            existe=true;
            this.des=(Descartes)q.list().get(0); //el primer elemento de la lista
