...
public class EmpresaGest {
   ...
   private Empresa emp=null; //guarda el objeto de la ultima busqueda
   ...
   public Empresa GetObjEmpresa(){
    ...
    public int getsiguientecodigo(){
        ...
        try{
            ...
            Query q= sesion.createQuery("select count(idempresa)from Empresa");
            if(Integer.parseInt(q.list().get(0).toString())==0){
                //cargarCampoPorDefecto();
                siguientecodigo=1;
            } else {
                Query qq= sesion.createQuery("select max(idempresa)from Empresa");
...
public boolean addEmpresa(Empresa obj){
    ...
    public boolean editEmpresa(Empresa obj){
      //devuelve true si la transaccion es exitosa
      //false si no lo es
    ...
    public boolean delEmpresa(Empresa obj){
    ...
    try{
        ...
        Query q=sesion.createQuery("from Empresa where idempresa = "+Integer.toString(id));
        if(q.list().isEmpty()){  //si no se encontro
            ...
            existe=true;
            this.emp=(Empresa)q.list().get(0); //el primer elemento de la lista
