...
public class FrmPais extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createMatteBorder(1, 1, 1, 1, new java.awt.Color(0, 0, 153)), "Datos del País", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12), new java.awt.Color(0, 0, 153))); // NOI18N
        ...
        jTextField1.setToolTipText("CÓDIGO DEL PAÍS");
        ...
        jTextField2.setToolTipText("DESCRIPCIÓN DEL PAÍS");
        ...
        Pais pais=new Pais();
        PaisGest paisges=new PaisGest();  //instanciamos las clases necesarias a lo largo del programa
    ...
    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked
        ...
        if(milista.isEmpty()==false){  //si no esta vacia la lista, es decir si hay algun registro en la tabla
            contador=1;
            pais=(Pais) (milista.get(contador-1));
            jTextField1.setText(Integer.toString(pais.getIdpais()));
        ...
        if(jTextField1.getText().isEmpty() || jTextField2.getText().isEmpty()){ //si el campo id o descripcion estan vacios
           JOptionPane.showMessageDialog(null,"Campos clave vacíos","ATENCIÓN",JOptionPane.INFORMATION_MESSAGE);
            ...
            if(Integer.parseInt(jTextField1.getText())>1){
                encontrado=paisges.ExisteIdEnTabla(Integer.parseInt(jTextField1.getText()));
                int r=0;
                boolean resu=false;
                if (encontrado){  //si se encontro y se desea EDITAR
                    ...
                    if (r==JOptionPane.YES_OPTION){ //si se presiono SI
                       // try {
                            //si se presiono SI
                            pai=paisges.GetObjPais();
                            pai.setDescrippais(jTextField2.getText().toUpperCase());
                            resu=paisges.editPais(pai);
                    ...
                    if (r==JOptionPane.YES_OPTION){ //si se presiono SI
                      //  try {
                            pai.setIdpais(Integer.parseInt(jTextField1.getText()));
                            pai.setDescrippais(jTextField2.getText().toUpperCase());
                            resu=paisges.addPais(pai);
                            if (resu){
                ...
                if (x==1){  //si se llevo a cabo algun cambio
                    ...
                    consulta=Sesion.createQuery("from Pais order by descrippais");
    ...
    private void jButton7MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton7MouseClicked
        ...
        if (encontrado && jTextField1.getText().isEmpty()==false){
            ...
            if (r==JOptionPane.YES_OPTION){ //si se presiono SI
                if(Integer.parseInt(jTextField1.getText())>1){
                  //  try {
                        if (monges.ExisteFKPais(Integer.parseInt(jTextField1.getText()))==false && clieges.ExisteFKPais(Integer.parseInt(jTextField1.getText()))==false){  //si no esta como FK en ninguna de las tablas
                            Pais pai=new Pais();
                            pai=paisges.GetObjPais();
                            resu=paisges.delPais(pai);
                            if (resu){  //SI SE EDITO CORRECTAMENTE
    ...
    private void jTextField1FocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jTextField1FocusLost
        ...
        if(jTextField1.getText().isEmpty()==false){
            encontrado=paisges.ExisteIdEnTabla(Integer.parseInt(jTextField1.getText()));
            if(encontrado){
                this.pais=paisges.GetObjPais();
                jTextField1.setText(Integer.toString(pais.getIdpais()));
                jTextField2.setText(pais.getDescrippais());
            } else {
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
       ...
       // setDefaultCloseOperation(0);
        ...
        if(paisges.getsiguientecodigo()==1)  //la primera vez carga la lista
            paisges.cargarListaPaises();
        Sesion=NewHibernateUtil.getSessionFactory().openSession();
        Sesion.beginTransaction();
        consulta=Sesion.createQuery("from Pais order by descrippais");
        milista=consulta.list();
